...
export function QuadrantPlot({ riskScore, readinessScore, quadrant, companyName }: QuadrantPlotProps) {
  ...
  // x = readiness (left=low, right=high), y = risk (bottom=low, top=high)
  const px = padding + (readinessScore / 100) * inner;
  const py = padding + ((100 - riskScore) / 100) * inner;

  const qConfig = QUADRANT_CONFIG[quadrant];

  return (
    <div className="flex flex-col items-center gap-3">
      <div className="text-xs font-semibold tracking-widest uppercase text-center" style={{ color: '#C5A572', fontFamily: 'Montserrat' }}>
        SOAR Quadrant
      </div>
      <div className="relative" style={{ width: plotSize, height: plotSize }}>
        ...
        <svg width={plotSize} height={plotSize} className="absolute inset-0">
          {/* Q2: RACE MODE (top-right, high risk, high readiness) */}
          ...
          {/* Axis labels */}
          <text x={plotSize/2} y={plotSize-8} textAnchor="middle" fill="#C5A572" fontSize="9" fontFamily="Montserrat" fontWeight="600">AI READINESS →</text>
          <text x={12} y={plotSize/2} textAnchor="middle" fill="#C5A572" fontSize="9" fontFamily="Montserrat" fontWeight="600" transform={`rotate(-90, 12, ${plotSize/2})`}>AI RISK →</text>

          {/* Company dot — eagle icon background circle */}
