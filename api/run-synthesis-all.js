...
// Bundled function: run-synthesis-all
let _netlifyExports = {};
"use strict";
...
var run_synthesis_all_exports = {};
__export(run_synthesis_all_exports, {
  handler: () => handler
});
_netlifyExports = __toCommonJS(run_synthesis_all_exports);
...
async function synthesizeOne(dimension, companyName, vertical, rawEvidence) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const evidenceCap = EVIDENCE_CAPS[attempt] ?? 8e3;
...
[NOTE: Total evidence was ${Math.round(evidenceLength / 1e3)}K chars. Showing first ${Math.round(evidenceCap / 1e3)}K.]` : "";
    const userPrompt = `Synthesize research evidence for ${companyName} (${vertical || "vertical SaaS"}) \u2014 dimension: "${dimension}".

Evidence: ${Math.round(Math.min(evidenceLength, evidenceCap) / 1e3)}K chars from 5 research passes.
...
Write a structured analyst brief following the numbered sections above. Label every fact. Be specific \u2014 name names, give numbers, cite sources.`;
    try {
      console.log(`[synth-all] ${dimension} attempt ${attempt + 1}/3: model=${model}, evidence=${Math.round(evidenceCap / 1e3)}K`);
...
var handler = async (event) => {
  ...
  try {
    ...
    console.log(`[synth-all] Corpora sizes: ${DIMENSIONS.map((d2) => `${d2}=${Math.round((corpora[d2] || "").length / 1e3)}K`).join(", ")}`);
    const results = await Promise.all(
      DIMENSIONS.map(
        (dim) => synthesizeOne(dim, company_name, vertical || "vertical SaaS", corpora[dim] || "")
      )
    );
    const briefs = {};
    const meta = {};
    DIMENSIONS.forEach((dim, i2) => {
      const r2 = results[i2];
      briefs[dim] = r2.brief;
      meta[dim] = {
        model_used: r2.model,
        elapsed_ms: r2.elapsed,
        synthesis_chars: r2.chars,
        fallback: r2.fallback
      };
    });
    const totalElapsed = Date.now() - globalStart;
    const successCount = results.filter((r2) => !r2.fallback).length;
...
module.exports = async function vercelHandler(req, res) {
  if (!_netlifyHandler) {
    res.status(500).json({ error: 'Handler not found in bundle', bundle: 'run-synthesis-all' });
    return;
  }
