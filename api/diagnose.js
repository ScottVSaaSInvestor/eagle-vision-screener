...
_netlifyExports = __toCommonJS(diagnose_exports);
...
var handler = async (event) => {
  ...
  if (!anthropicKey) {
    ...
    try {
      ...
      const reply = response.content.find((b2) => b2.type === "text")?.text ?? "";
      ...
      results.checks.anthropic = {
        ...
        key_prefix: `${anthropicKey.slice(0, 12)}...`,
        hint: e2?.status === 401 ? "Invalid API key \u2014 check ANTHROPIC_API_KEY in Netlify env vars" : e2?.status === 429 ? "Rate limited \u2014 too many requests" : e2?.message?.includes("model") ? `Model "${anthropicModel}" not found \u2014 check ANTHROPIC_MODEL` : ""
      };
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        ...
        results.checks.tavily = {
          ...
          hint: res.status === 401 ? "Invalid Tavily API key" : res.status === 429 ? "Tavily rate limit exceeded" : ""
        };
      }
    } catch (e2) {
      results.checks.tavily = {
  ...
  if (!braveKey) {
    ...
    try {
      ...
      results.checks.brave = { status: "ERROR", error: e2?.message };
    }
  }
  results.checks.passcode = process.env.ACCESS_PASSCODE ? { status: "SET", length: process.env.ACCESS_PASSCODE.length } : { status: "MISSING", message: "ACCESS_PASSCODE not set \u2014 login will not work" };
  const allOk = results.checks.anthropic?.status === "OK" && results.checks.tavily?.status === "OK";
  results.summary = allOk ? "\u2705 All critical services operational \u2014 Eagle Vision should work correctly" : "\u274C One or more critical services are not configured correctly \u2014 see checks above";
...
module.exports = async function vercelHandler(req, res) {
  if (!_netlifyHandler) {
    res.status(500).json({ error: 'Handler not found in bundle', bundle: 'diagnose' });
    return;
  }
