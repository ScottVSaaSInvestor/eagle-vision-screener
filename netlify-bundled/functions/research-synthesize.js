...
var handler = async (event) => {
  ...
  try {
    ...
    const evidenceToSynth = (raw_evidence || "No evidence provided \u2014 use analyst training knowledge").slice(0, 85e3);
    const truncationNote = evidenceLength > 85e3 ? `

[NOTE: Total evidence was ${Math.round(evidenceLength / 1e3)}K chars. Showing first 85K chars to synthesizer. Remaining ${Math.round((evidenceLength - 85e3) / 1e3)}K chars not shown \u2014 ensure the most critical sources were ranked first.]` : "";
...
EVIDENCE VOLUME: ${Math.round(evidenceLength / 1e3)}K characters collected across 5 research passes (broad search, deep crawl, gap analysis, gap fill, second pass).
...
Target length: 1500-2500 words. Be thorough \u2014 this is the backbone of the investment analysis.`;
    ...
    while (attempt < 3) {
      ...
      try {
        ...
        if (attempt < 3) await new Promise((r2) => setTimeout(r2, 2e3 * attempt));
      }
    }
