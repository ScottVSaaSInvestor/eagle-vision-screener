...
var PackOutputSchema = external_exports.object({
  pack_name: external_exports.literal("company_profile"),
  pack_version: external_exports.literal("1.0"),
  generated_at: external_exports.string(),
  data_quality_score: external_exports.number().min(0).max(1),
  findings: external_exports.array(external_exports.object({
    key: external_exports.string(),
    value: external_exports.unknown(),
    confidence: external_exports.enum(["H", "M", "L"]),
    sources: external_exports.array(external_exports.string()),
    unknowns: external_exports.array(external_exports.string())
  })),
  factor_inputs: external_exports.record(external_exports.object({
    evidence_summary: external_exports.string(),
    signal_strength: external_exports.number().min(0).max(1)
  })),
  red_flags: external_exports.array(external_exports.string()),
  v2_stub: external_exports.literal(false)
});
var handler = async (event) => {
  ...
  try {
    ...
    const evidenceContext = Array.isArray(evidence_texts) ? evidence_texts.slice(0, 4).join("\n\n---\n\n") : "";
    const docContext = document_text ? `

UPLOADED DOCUMENT EXCERPT:
${document_text.slice(0, 3e3)}` : "";
